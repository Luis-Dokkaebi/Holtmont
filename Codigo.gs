...
function apiLogin(username, password) {
  ...
  // [REMOVIDO POR SEGURIDAD] - El acceso '123' ha sido deshabilitado.
  // Si se requiere acceso genérico, debe implementarse mediante USER_DB o un método seguro.
  ...
  try {
    ...
    tasksArray.forEach(task => {
      ...
      if (rowIndex === -1 && task._rowIndex) {
         // Safety check: ensure we are not overwriting a different ID if using _rowIndex fallback
         const fallbackIndex = parseInt(task._rowIndex) - 1;
         if (fallbackIndex > headerRowIndex && fallbackIndex < values.length) {
            // If the row has an ID and it's different from our task ID, do NOT use this row.
            // This prevents overwriting if rows shifted.
            if (folioIdx > -1 && tFolio) {
               const rowId = String(values[fallbackIndex][folioIdx]).toUpperCase().trim();
               if (rowId && rowId !== tFolio) {
                   console.warn("ID mismatch at index " + fallbackIndex + ". Expected " + tFolio + " but found " + rowId + ". Treating as new row.");
                   rowIndex = -1;
               } else {
                   rowIndex = fallbackIndex;
               }
            } else {
               // If no ID involved, we trust the index (or we could be stricter)
               rowIndex = fallbackIndex;
            }
         }
      }
...
function apiUpdateTask(personName, taskData) {
    try {
        ...
        // Sync to ADMINISTRADOR (Master) if not already updating Master
        if (String(personName).toUpperCase() !== "ADMINISTRADOR") {
             const syncObj = { ...taskData };
             delete syncObj._rowIndex; // Remove index as it applies to local sheet
             try { internalBatchUpdateTasks("ADMINISTRADOR", [syncObj]); } catch(e){ console.error("Sync error", e); }
        }

        // Special handling for TONITA (Preserved existing logic for pushing down to Salesperson)
